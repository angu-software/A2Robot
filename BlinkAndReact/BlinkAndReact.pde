...
#define WHEEL_LEFT_MAX_FORWARD    20
#define WHEEL_RIGHT_MAX_FORWARD   160
...
Movement stepperDirection = forwards;
int mStepperAngle;
...
void loop() {
    ...
    if (mState == STATE_FOLLOW_FRIEND) {
        if (!mTimerRuns) {
            ...
            if (mFriendLastSeenAngle < 45) {
                driveRobot(full, forwards, left);
            } else if (mFriendLastSeenAngle > 55) {
